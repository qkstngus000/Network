...
int connectServer() {
    ...
    while (TRUE) {
        ...
        if (getline(&send_buffer, &buffersize, stdin) < 0) perror("message write failed");
        printf("%s", send_buffer);
        if (write(client_socket_desc, &send_buffer, sizeof(char)) < 0) perror("write to server failure");
